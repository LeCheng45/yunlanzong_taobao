...
public interface GoodsInfoMapper extends BaseMapper<GoodsInfo> {
    ...
    @Select("select `goods_cover_img` from goods_info where `goods_id`= #{goods_id}")
    String selectUrlById(@Param("goods_id") Long goods_id);

    @Select("select `store_id` from goods_info where `goods_id`= #{goods_id}")
    String selectStoreById(@Param("goods_id") Long goods_id);
